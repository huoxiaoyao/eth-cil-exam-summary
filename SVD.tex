...
With users on rows, movies on cols. $\mathbf{U}$: user to factor embeddings (prototipical movie on cols), $\mathbf{V}^\top$ items to factor (prot user on cols), $\mathbf{D}$ strength of factors (k singular values $<= rank(\mathbf{A})$
