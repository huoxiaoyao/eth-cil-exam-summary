...
\begin{compactdesc}
	...
	\item Jensen's $log(\mathbb{E}[\mathbb{X}]) >= \mathbf{E}[log(\mathbf{X})]$
\end{compactdesc}
...
\begin{inparaitem}
	...
	\item $P(x_1, \ldots, x_n) = \prod_{i=1}^n P(x_i)$ (iff IID) \item $D_{KL}(P||Q)=\int_{-\infty}^{\infty}p(x)\log{\frac{p(x)}{q(x)}}dx$
\end{inparaitem}